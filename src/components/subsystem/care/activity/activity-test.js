...
describe('i2web/components/subsystem/care/activity', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-subsystem-care-activity {subsystem}="subsystem"/>',
      scope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-subsystem-care-activity').children().length, 1, 'arcus-subsystem-care-activity is rendered');
    });
  });
});
