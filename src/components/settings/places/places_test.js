...
describe('i2web/components/settings/places/places', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-settings-places {places}="places" />',
      scope: {
        places,
      },
      appScope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.lengthOf($('arcus-settings-places'), 1, 'arcus-settings-places is rendered');
    });
  });
});
