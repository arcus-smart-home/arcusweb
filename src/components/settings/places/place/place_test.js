...
describe('i2web/components/settings/places/place', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-settings-place {place}="place" />',
      scope: {
        place: places[0],
        places,
      },
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.lengthOf($('arcus-settings-place'), 1, 'arcus-settings-place is rendered');
    });
  });
});
