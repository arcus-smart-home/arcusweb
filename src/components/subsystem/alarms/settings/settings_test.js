...
describe('i2web/components/subsystem/alarms/settings', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-subsystem-alarms-settings />',
      scope: {},
      appScope: {
        subsystems,
      },
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.lengthOf($('arcus-subsystem-alarms-settings'), 1, 'arcus-subsystem-alarms-settings is rendered');
    });
  });
});
