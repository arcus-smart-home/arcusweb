...
describe('i2web/components/subsystem/care/settings', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-subsystem-care-settings {subsystem}="subsystem"/>',
      scope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-subsystem-care-settings').children().length, 1, 'arcus-subsystem-care-settings is rendered');
    });
  });
});
