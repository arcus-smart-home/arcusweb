...
describe('i2web/pages/invite', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-pages-invite {message}="message" />',
      scope: {
        message: 'This is the arcus-pages-invite component',
      },
      appScope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-pages-invite').children().length, 1, 'arcus-pages-invite is rendered');
    });
  });
});
