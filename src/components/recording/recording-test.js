...
describe('i2web/components/recording', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-recording {recording}="recording" />',
      scope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.lengthOf($('arcus-recording'), 1, 'arcus-recording is rendered');
    });
  });
});
