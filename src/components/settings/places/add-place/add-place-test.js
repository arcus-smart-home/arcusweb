...
describe('i2web/components/settings/places/add-place', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-settings-add-place />',
      scope: {
      },
      appScope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-settings-add-place').children().length, 1, 'arcus-settings-add-place is rendered');
    });
  });
});
