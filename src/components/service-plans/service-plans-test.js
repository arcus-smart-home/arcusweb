...
describe('i2web/components/service-plans', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-service-plans {message}="message" />',
      scope: {
        message: 'This is the arcus-service-plans component',
      },
      appScope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-service-plans').children().length, 1, 'arcus-service-plans is rendered');
    });
  });
});
