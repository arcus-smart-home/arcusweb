...
import { ViewModel } from './irrigation-event';

let cleanupAfterRender;

describe('i2web/components/schedule/irrigation-event', function favorite() {
  describe('functional tests', function functional() {
    ...
    beforeEach(function beforeEach(done) {
      loginAndRender({
        renderTo: '#test-area',
        template: '<arcus-schedule-irrigation-event {message}="message" />',
        scope: {
          message: 'This is the arcus-schedule-irrigation-event component',
        },
        appScope: {
    ...
    describe('rendering', function rendering() {
      it('shall be rendered on the page', function isRendered() {
        assert.lengthOf($('arcus-schedule-irrigation-event'), 1, 'arcus-schedule-irrigation-event is rendered');
      });
    });
  });
