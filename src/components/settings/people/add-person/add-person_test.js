...
describe('i2web/components/settings/people/add-person', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-settings-people-add-person {place}="place" />',
      scope: {
        place: Places[0],
      },
    }).then(({ cleanup }) => {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.lengthOf($('arcus-settings-people-add-person'), 1, 'arcus-settings-people-add-person is rendered');
    });
  });
});
