...
describe('i2web/components/subsystem/care/card', function favorite() {
  ...
  beforeEach(function beforeEach(done) {
    loginAndRender({
      renderTo: '#test-area',
      template: '<arcus-subsystem-card-care {subsystem}="subsystem"/>',
      scope: {
  ...
  describe('rendering', function rendering() {
    it('shall be rendered on the page', function isRendered() {
      assert.isAtLeast($('arcus-subsystem-card-care').children().length, 1, 'arcus-subsystem-card-care is rendered');
    });
  });
});
